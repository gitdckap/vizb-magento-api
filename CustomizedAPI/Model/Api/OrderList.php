...
class OrderList implements \DCKAP\CustomizedAPI\Api\OrderListInterface
{
    ...
    public function getOrderData($totalOrderCollection, $orderCollection)
    {
        ...
        try {
            foreach ($orderCollection as $order) {
                ...
                $orderDetails->setTotalPaid(round($order->getTotalPaid(), 2));
                $orderDetails->setTotalRefunded(round($order->getTotalRefunded(), 2));
                $orderDetails->setShippingRefunded(round($order->getShippingRefunded(), 2));
                $orderDetails->setStatusHistories($order->getStatusHistories());
                $orderDetails->setPayment($order->getPayment());
                ...
                foreach ($items as $item) {
                    ...
                    $orderItem->setQtyRefunded(round($item->getQtyRefunded(),2));
                    $orderItem->setQtyReturned(round($item->getQtyReturned(),2));
                    $itemsArr[] = $orderItem; 
                }
            ...
            $orderResponse->setOverallOrdercount($totalOrderCollection);
    ...
    public function getRecentOrderList($id)
    {
        ...
        $select     = $connection->select()
                                ->from($resource->getTableName('sales_order'), 'COUNT(entity_id)')
                                ->where('entity_id>?', $id);
        $totalOrderCollection = (int) $connection->fetchOne($select);
